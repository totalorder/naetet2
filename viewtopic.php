...
if($start == -1){
	$start = $total_posts - $config['posts_per_page'];
}
...
$info = array(
	...
	'previous_page' => $start > 0 ? max(0, $start - $config['posts_per_page']) : -1,
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	{
//		echo("SECOND PASS!" .var_dump($bbcode). $row['bbcode_bitfield'] . "Bitfield" . $bbcode_bitfield);
...
header('Content-Type: application/json');
$jsonp_callback_name = request_var('callback', 'callback');
echo($jsonp_callback_name . "(" . json_encode(array('info' => $info, 'posts' => $posts)) . ");");
// Output the page
