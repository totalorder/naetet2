...
//		$db->sql_return_on_error(false);
	...
	*/
	...
	function set_cookie_http($name, $cookiedata, $cookietime)
	{
		global $config;
		...
		header('Set-Cookie: ' . $name_data . (($cookietime) ? '; expires=' . $expire : '') . '; path=' . $config['cookie_path'] . $domain . ((!$config['cookie_secure']) ? '' : '; secure') . ';', false);
	}
